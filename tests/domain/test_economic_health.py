...
def test_health_preserves_profitability_and_stability_metrics():
    ...
    result = calculate_economic_health(history, EconomicPerformanceWindow(datetime(2026, 1, 1), datetime(2026, 1, 5)))
    ...
    assert result.window == EconomicPerformanceWindow(datetime(2026, 1, 1), datetime(2026, 1, 5))
...
def test_single_outcome_has_no_stability_deviation():
    result = calculate_economic_health(
        ...
        EconomicPerformanceWindow(datetime(2026, 1, 1), datetime(2026, 1, 5)),
    )
    assert result.sample_profit_standard_deviation is None


def test_empty_history_returns_no_economic_health():
    assert calculate_economic_health(EconomicPerformanceHistory(business_id="b1"), EconomicPerformanceWindow(datetime(2026, 1, 1), datetime(2026, 1, 5))) is None
