...
def stat(interpretation, eligible=True):
    return StatisticalEvidenceComposition(business_id="b1",metric_name="profit",unit="EGP",method="welch_two_sample_t_test",observation_ids=("b","c"),eligible=eligible,reason=StatisticalEvidenceEligibilityReason.ELIGIBLE if eligible else StatisticalEvidenceEligibilityReason.INSUFFICIENT_EVIDENCE_QUALITY,interpretation=interpretation,alpha=.05)
